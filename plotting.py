...
def plot_population_results(experiments: List[Dict[str, Any]], steps: int, filename: str = "./output/figure_population.png") -> None:
    x = np.arange(steps)
...
def plot_final_state_and_lineages(experiments: List[Dict[str, Any]], filename: str = "./output/figure_lineages.png") -> None:
    ...
    for ax, exp in zip(axes, experiments):
        ...
        if H.shape[1] != 2:
            raise ValueError("This plotting helper expects dim=2.")
        ...
        ax.text(0.02, 0.98, f"N={H.shape[0]}\nL={len(uniq)}", transform=ax.transAxes,
